...
console.log(
	`[smoke] using ports: start_app=${DEBUG_PORT} attach=${ATTACH_PORT}`,
);
...
async function main() {
	...
	try {
		...
		assert(
			...
			`console error: ${consoleResult.content?.[0]?.text}`,
		);
		...
		assert(
			!networkResult.isError,
			`network error: ${networkResult.content?.[0]?.text}`,
		);
		const networkData = parseToolText(networkResult);
		assert((networkData.entries ?? []).length > 0, "expected network entries");
		...
		assert(
			!domResult.isError,
			`get_dom error: ${domResult.content?.[0]?.text}`,
		);
		...
		assert(
			!pageInfoResult.isError,
			`page_info error: ${pageInfoResult.content?.[0]?.text}`,
		);
		const pageInfo = parseToolText(pageInfoResult);
		assert(
			pageInfo.title?.includes("Minimal Electron Fixture"),
			"bad page title",
		);
		pass("page_info");
		...
		assert(
			!typeResult.isError,
			`type_text error: ${typeResult.content?.[0]?.text}`,
		);
		pass("type_text");
		...
		assert(
			!clickResult.isError,
			`click error: ${clickResult.content?.[0]?.text}`,
		);
		pass("click");
		...
		assert(
			!waitResult.isError,
			`wait_for error: ${waitResult.content?.[0]?.text}`,
		);
		pass("wait_for");
		...
		assert(
			!pressResult.isError,
			`press_key error: ${pressResult.content?.[0]?.text}`,
		);
		pass("press_key");
		...
		assert(
			!liveResult.isError,
			`set_console_live error: ${liveResult.content?.[0]?.text}`,
		);
		...
		assert(
			!saveResult.isError,
			`save_screenshot error: ${saveResult.content?.[0]?.text}`,
		);
		const saved = parseToolText(saveResult);
		assert(
			saved.path && fs.existsSync(saved.path),
			`screenshot file missing: ${JSON.stringify(saved)}`,
		);
		...
		assert(
			!clipResult.isError,
			`element screenshot error: ${clipResult.content?.[0]?.text}`,
		);
		const clipped = parseToolText(clipResult);
		assert(
			clipped.clip?.selector === "#heading",
			`expected clip meta: ${JSON.stringify(clipped)}`,
		);
		...
		assert(
			!setStore.isError,
			`set_storage error: ${setStore.content?.[0]?.text}`,
		);
		...
		assert(
			!getStore.isError,
			`get_storage error: ${getStore.content?.[0]?.text}`,
		);
		const store = parseToolText(getStore);
		assert(
			store.entries?.smoke === "1",
			`storage mismatch: ${JSON.stringify(store)}`,
		);
		...
		if (!setCookieResult.isError) {
			...
			assert(
				!cookiesResult.isError,
				`get_cookies error: ${cookiesResult.content?.[0]?.text}`,
			);
			pass("get/set_cookie");
		} else {
			const msg = setCookieResult.content?.[0]?.text || "";
			assert(
				/cookie|url|domain|success|false|Invalid/i.test(msg) || msg.length > 0,
				msg,
			);
		...
		assert(
			!traceStart.isError,
			`start_tracing error: ${traceStart.content?.[0]?.text}`,
		);
		...
		assert(
			!traceStop.isError,
			`stop_tracing error: ${traceStop.content?.[0]?.text}`,
		);
		const trace = parseToolText(traceStop);
		assert(
			fs.existsSync(trace.path || tracePath),
			`trace file missing: ${JSON.stringify(trace)}`,
		);
		...
		assert(
			!findResult.isError,
			`find_apps error: ${findResult.content?.[0]?.text}`,
		);
		const foundApps = parseToolText(findResult);
		assert(
			(foundApps.apps ?? []).some(
				(a) =>
					a.debugPort === DEBUG_PORT ||
					/minimal-electron-app/.test(a.command || ""),
			),
		...
		if (startedPid) {
			...
			if (!byPid.isError) {
				...
				assert(
					/debug|port|pid|Could not resolve/i.test(msg),
					`unexpected attach_by_pid error: ${msg}`,
				);
		...
		assert(
			!waitEnabled.isError,
			`wait_for enabled error: ${waitEnabled.content?.[0]?.text}`,
		);
		...
		assert(
			!waitCount.isError,
			`wait_for count error: ${waitCount.content?.[0]?.text}`,
		);
		pass("wait_for count");
		...
		assert(
			!clearResult.isError,
			`clear_buffers error: ${clearResult.content?.[0]?.text}`,
		);
		pass("clear_buffers");
		...
		assert(
			!stopResult.isError,
			`stop_app error: ${stopResult.content?.[0]?.text}`,
		);
		pass("stop_app");
		processId = undefined;
