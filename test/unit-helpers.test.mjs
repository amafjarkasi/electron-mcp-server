...
test("parseDebugPortFromCommand parses --remote-debugging-port=PORT", () => {
	assert.equal(
		parseDebugPortFromCommand(
			"/usr/bin/electron --remote-debugging-port=9222 ./app",
		),
		9222,
	);
});
...
test("parseInspectPortFromCommand is case-insensitive", () => {
	assert.equal(parseInspectPortFromCommand("electron --INSPECT=9231"), 9231);
});
...
test("isDevToolsTarget identifies the DevTools front-end", () => {
	...
	assert.equal(
		isDevToolsTarget({ url: "http://localhost:5174/chat/x" }),
		false,
	);
...
test("pickTargetByRole prefers the app page over an open DevTools window", () => {
	// `evaluate` routes an explicitly requested role here rather than through
	// pickPageTarget, so the preference has to live in both — a documented
	// `role: "page"` call would otherwise still drive the debugger's own UI.
	const proc = makeProc({
		targets: makeTargets([
			{
				id: "devtools",
	...
	assert.equal(pickTargetByRole(proc, "page").id, "app");
});

test("pickTargetByRole falls back to DevTools when it is the only page", () => {
	const proc = makeProc({
		targets: makeTargets([
			{
				id: "devtools",
	...
	assert.equal(pickTargetByRole(proc, "page").id, "devtools");
});

test("pickTargetByRole still throws when no target has the role", () => {
	const proc = makeProc({
		targets: makeTargets([
			{ id: "app", type: "page", title: "App", url: "http://localhost:5174/" },
		]),
	});
	assert.throws(() => pickTargetByRole(proc, "worker"), /No worker target/);
});
...
test("pickTargetByRole delegates to pickPageTarget when targetId given", () => {
	...
	assert.throws(() => pickTargetByRole(proc, "page", "nope"), /not found/);
});
...
test("validateOutputPath blocks the home .ssh directory", () => {
	...
	try {
		...
		assert.throws(() => validateOutputPath(sshPath), /sensitive location/i);
	} finally {
		restoreEnv(prev);
	}
});
...
test("validateOutputPath respects ELECTRON_MCP_OUTPUT_ROOTS allowlist", () => {
	const prev = setEnv(
		isWin ? "C:\\tmp\\out;C:\\tmp\\traces" : "/tmp/out|/tmp/traces",
	);
	try {
...
test("validateOutputPath allowlist still blocks sensitive locations even when inside a root", () => {
	...
	try {
		...
		assert.throws(() => validateOutputPath(blocked), /sensitive location/i);
	} finally {
		restoreEnv(prev);
	}
});
...
test("validateOutputPath splits allowlist on both ';' and '|'", () => {
	...
	try {
		assert.ok(
			validateOutputPath(isWin ? "C:\\tmp\\a\\1" : "/tmp/a/1").length > 0,
		);
		assert.ok(
			validateOutputPath(isWin ? "C:\\tmp\\b\\2" : "/tmp/b/2").length > 0,
		);
	} finally {
		restoreEnv(prev);
	}
		...
		targets: makeTargets([
			{
				id: "a",
				type: "page",
				webSocketDebuggerUrl: "ws://127.0.0.1:9/page/a",
			},
...
test("clampClipToViewport leaves a fractional rect inside the viewport alone", () => {
	...
	const box = {
		x: 384.7548828125,
		y: 58.046939849853516,
		width: 916.9063110351562,
		height: 383.994140625,
	};
...
test("clampClipToViewport reports a sub-pixel crop rather than rounding it away", () => {
	...
	const left = clampClipToViewport({
		x: -0.5,
		y: 10,
		width: 200,
		height: 100,
	...
	const right = clampClipToViewport({
		x: 0,
		y: 0,
		width: 1900.4,
		height: 999.6,
