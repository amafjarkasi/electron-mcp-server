...
export async function probeDebugPort(
	...
	try {
		...
		const targets = listRes.ok ? ((await listRes.json()) as CDPTarget[]) : [];
...
export async function captureScreenshot(
	...
	const resetPageSocket = async () => {
		...
		if (electronProcess.cdpTargetId === target.id) {
			if (electronProcess.cdpClient && electronProcess.cdpClient !== mon) {
				await closeClient(electronProcess.cdpClient);
			}
			electronProcess.cdpClient = undefined;
			electronProcess.cdpTargetId = undefined;
	...
	try {
		...
		log.warn(
			`[${electronProcess.id}] screenshot failed, one more reconnect:`,
			err,
		);
		await resetPageSocket();
...
export function pickTargetByRole(
	...
	if (!match) {
		throw new Error(
			`No ${role} target found for process ${electronProcess.id}`,
		);
	}
	return match;
}
...
export async function navigatePage(
	...
	if (waitUntilLoad) {
		...
		if (!loaded) {
			throw new Error(
				`navigate to ${url} timed out after ${timeoutMs}ms waiting for page load`,
			);
		}
	}
...
export async function waitForCondition(
	...
	while (Date.now() - started < timeoutMs) {
		...
		if (options.selector) {
			...
			if (found) matches.push(`selector:${options.selector}`);
		}
		...
		if (options.count) {
			...
			if (n >= options.count.min) {
				matches.push(
					`count:${options.count.selector}>=${options.count.min} (got ${n})`,
				);
			}
		}
		...
		if (options.text) {
			...
			if (found) matches.push(`text:${options.text}`);
		}
		...
		if (options.urlIncludes) {
			...
			if (url.includes(options.urlIncludes)) {
				matches.push(`url:${options.urlIncludes}`);
			}
		}
		...
		let expectedCount = 0;
		if (options.selector) expectedCount++;
		if (options.hidden) expectedCount++;
		if (options.enabled) expectedCount++;
		if (options.count) expectedCount++;
		if (options.text) expectedCount++;
		if (options.urlIncludes) expectedCount++;
		if (options.consoleIncludes) expectedCount++;
...
export async function resolveDebugPortForPid(
	...
	if (process.platform === "linux") {
		try {
			...
			const ports = Array.from(stdout.matchAll(/:(\d+)\s/g), (m) =>
				Number(m[1]),
			).filter((p) => p >= 1024);
